...
public class Renderer {
  ...
  public static Image image_border_top;
  ...
  public static void init() {
    ...
    try {
      ...
      image_border_top = ImageIO.read(Launcher.getResource("/assets/hbar/border0.png"));
  ...
  public static void resize(int w, int h) {
    ...
    new_size.height =
        Math.max(
            h, 357 - GAME_RENDER_OFFSET); // actual height that could be used for game rendering
  }
  ...
  public static void handle_resize() {
    ...
    if (Renderer.instance != null && Reflection.setGameBounds != null) {
      try {
        Reflection.setGameBounds.invoke(Renderer.instance, 0, 0, Renderer.width, Renderer.height);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  public static void present(Image image) {
    ...
    if (Client.state == Client.STATE_GAME) {
      ...
      if (!Client.isInterfaceOpen() && Settings.SHOW_BUFFS.get(Settings.currentProfile)) {
        ...
        for (int i = 0; i < 18; i++) {
          if (Client.current_level[i] != Client.base_level[i] && (i != Client.SKILL_HP)) {
      ...
      if (Settings.RSCTIMES_BUTTONS_FUNCTIONAL.get(Settings.currentProfile)
        ...
        /*|| Settings.HISCORES_LOOKUP_BUTTON.get(Settings.currentProfile)*/ ) {
          mapButtonBounds = new Rectangle(width - 68 - 99, 3 + GAME_RENDER_OFFSET, 32, 32);
          if (Settings.SHOW_RSCTIMES_BUTTONS.get(Settings.currentProfile)) {
            g2.setColor(Renderer.color_text);
            g2.drawLine(
                mapButtonBounds.x + 1,
          ...
          if (bufferedMouseClick.getX() >= mapButtonBounds.x
            ...
            if (bufferedMouseClick.isRightClick()) {
              ...
              } else*/ if (Settings.MOTIVATIONAL_QUOTES_BUTTON.get(
      ...
      if (Settings.SHOW_PLAYER_POSITION.get(Settings.currentProfile)) {
        ...
        if ((!Replay.isPlaying || screenshot)) offset += 60;
        if (Client.shouldHideFPS()) offset -= 60;
        drawShadowText(
            g2,
      ...
      // TODO: when replay functionality is placed should check;
      boolean wantToDrawServerReplayButtons = false; /*
